...
- Sabendo o tipo de falha que há, fomos direto tentar verificar via inspeção de elementos se havia algo relacionado ao GUID ou algum arquivo aberto, mas sem resultado.
- Levando em conta que o ID fica exposto na URL, usamos isso mudando a URL de `wiener` para `administrator`.
- Por surpresa (quase óbvia), era este o segredo para acessar, mas ainda faltava obter a senha do usuário admin.
